$(document).ready(function () {
  ...
  $("#pay_btn").on("click", async function (e) {
    e.preventDefault();
    ...
    if (selectedTheatreIndex !== -1) {
      ...
      if (selectedShowtimeIndex !== -1) {
        ...
        await updateMoviesJson(movieData);
        await saveBookingToDb();
        selectedSeats = [];

        // window.location.href = "../../HTML/feedback_form.html";
  ...
  function saveBookingToDb() {
    ...
    axios
      ...
      .then((response) => {
        ...
        window.location.replace("../../HTML/feedback.html");
      })
      .catch((error) => {
